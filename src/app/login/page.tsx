...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/auth/login', {
        ...
        body: JSON.stringify({ registration_number: identifier, email: identifier, password }),
      });
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'linear-gradient(160deg, #1A2344 0%, #0D1628 60%, #1A2344 100%)' }}>
        ...
        <div style={{ width: '100%', maxWidth: 420 }}>
          ...
          <div style={{
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              {error && (
                <div style={{ background: 'rgba(200,16,46,0.15)', border: '1px solid rgba(200,16,46,0.4)', borderRadius: '0.4rem', padding: '0.6rem 0.8rem', color: '#FF8A80', fontSize: '0.83rem' }}>
                  {error}
                </div>
              )}
              ...
              <button
                ...
                style={{
                  width: '100%',
                  background: loading ? '#8B0000' : '#C8102E',
                  color: 'white',
                  padding: '0.8rem',
                  borderRadius: '0.5rem',
                  border: 'none',
