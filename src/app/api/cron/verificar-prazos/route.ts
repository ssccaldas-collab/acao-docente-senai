...
import { sendDeadlineReminderEmail, sendOverdueEmail } from '@/lib/mailer';
...
export async function GET(req: NextRequest) {
  ...
  // 1. Lembrete ao docente: prazo da Etapa 1 vencendo em 3 dias
  const upcomingStage1 = await sql`
    ...
    WHERE ec.current_stage = 1
      ...
      AND ec.stage1_deadline = CURRENT_DATE + INTERVAL '3 days'
      AND NOT EXISTS (
        SELECT 1 FROM notification_log nl
        WHERE nl.cycle_id = ec.id AND nl.notification_type = 'stage1_deadline_reminder'
      )
  `;
  for (const row of upcomingStage1 as { id: number; email: string; name: string; stage1_deadline: string }[]) {
    await sendDeadlineReminderEmail(row.email, row.name, new Date(row.stage1_deadline).toLocaleDateString('pt-BR', { timeZone: 'UTC' }));
    await sql`
      INSERT INTO notification_log (cycle_id, recipient_id, notification_type)
      SELECT ${row.id}, teacher_id, 'stage1_deadline_reminder' FROM evaluation_cycles WHERE id = ${row.id}
  ...
  const overdue = await sql`
    SELECT ec.id, ec.current_stage, ec.manager_id, m.email as manager_email, u.name as teacher_name,
  ...
  for (const row of overdue as { id: number; current_stage: number; manager_id: number | null; manager_email: string | null; teacher_name: string; current_deadline: string | null }[]) {
